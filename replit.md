...
### Industry Pages
- `retail.astro` - Retail solutions (redesigned, dark navy/blue/cyan design system). Premium SaaS layout: Hero with "PPD Retail — Dashboard" mockup (revenue/transactions/basket/returns KPIs, sparkline chart, floating POS checkout and Inventory Alert cards), metrics bar (4 stats), 6 feature cards (Omnichannel Checkout, Real-Time Inventory, Loyalty & CRM, BOPIS, Returns, Security), 4-card software showcase section (POS & Smart Checkout with bar chart, Inventory Visibility with status rows, Self-Checkout & Kiosks with mock UI, Online + In-Store Sync with animated arrow), 5 hardware cards with local images (VP800, Pinpads, P18, NRS POS, Pays POS — Clover Duo removed), and CTA with radial glow. Uses `rt-` CSS prefix, scoped styles. All neocities.org links removed, uses `/images/...` local paths. No `is:global`.
