...
- `repair.astro` - PPDrepair SaaS product landing page (`rp-*` CSS prefix). Branded as "PPDrepair" throughout. Hero with layered dashboard mockup + 4 floating cards. Industries section (8 repair types). 6-step guided interactive workflow demo. **Module Showcase**: 6-slide carousel (POS, Inventory, Team & Stores, Reports & Analytics, Customers/CRM, Payments/Terminal) — each slide shows sales copy (left) + interactive mini demo panel (right) with tab-switching, toggle buttons, and realistic UI. CSS in external `src/styles/repair-slider.css` (avoids Astro scoping). "Why Switch" comparison, $30/month pricing, lead capture modal with focus trap. ARIA tablist + roving tabindex. Scoped `<style>` and `<script>` (no `is:global`).
