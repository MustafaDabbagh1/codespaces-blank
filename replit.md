...
- `signup.astro` - Merchant Processing Application (fully redesigned). Premium fintech-style onboarding page with dark navy/blue/cyan design system. Sections: Hero with "PPD Application Portal" dashboard mockup (progress checklist, status cards, floating 256-bit encryption and Next-Day Funding cards), trust strip (6 credibility items), "What You'll Need" preparation cards (6 cards), main 6-step multi-step form (Business Info, Owner/Principal, Processing Profile, Banking with secure banner, Equipment selection with visual card grid, Review & Submit with auto-generated summary), "Why Businesses Apply" value cards (6 cards), FAQ accordion (6 items), CTA band, and sticky mobile apply button. Form features: progress bar stepper, field validation with `.su-invalid` class, account number confirmation, additional owner disclosure toggle, review summary auto-build, Formspree submission, success state with 3-step next-steps, consent checkboxes. Uses `su-` CSS prefix, scoped styles. No `is:global`.
